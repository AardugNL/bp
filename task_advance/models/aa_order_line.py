...
class SaleOrderLine(models.Model):
    ...
    aa_capacity_machine_id = fields.Many2one('aa_capacity.machine',
    ...
    def aa_compute_total_production_time(self):
        for aa_record  in self:
            aa_record.total_production_time = aa_record.product_uom_qty * aa_record.product_id.aa_production_time_1
    ...
    def aa_compute_production_date(self):
        for aa_rec in self:
            if aa_rec.order_id.confirmation_date and aa_rec.product_id.aa_product_lead_time:
                aa_rec.production_date = aa_rec.order_id.confirmation_date + datetime.timedelta(
            ...
            if aa_rec.aa_production_date and aa_rec.aa_resource_id:
                ...
                if not aa_machine_capacity:
                    ...
                    aa_machine_capacity.write({
                        'aa_name' : aa_machine_capacity._join_name_date(aa_rec.aa_resource_id.aa_name, aa_rec.aa_production_date.date())
                    })
